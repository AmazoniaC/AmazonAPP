...
// ── Build HTML quotation ──────────────────────────────────────────────────────
function buildQuotationHtml({ quotation, customer, settings }) {
  const fmt = (n) =>
    '$ ' + Number(n).toLocaleString('es-CO', { minimumFractionDigits: 0, maximumFractionDigits: 0 })
  const green = '#152E2A'
  const concreteBg = 'linear-gradient(120deg,#888 0%,#b0aea8 20%,#ccc9c4 45%,#b5b2ad 70%,#909090 100%)'
  ...
  const logoHtml = settings.logo
    ? `<img src="${settings.logo}" alt="Logo" style="height:110px;width:auto;object-fit:contain;">`
    : `<div style="text-align:right;"><div style="font-size:28px;font-weight:900;color:${green};letter-spacing:2px;">${(settings.companyName || 'Amazonia Concrete').toUpperCase()}</div><div style="font-size:10px;letter-spacing:4px;color:#444;border-top:1px solid #666;padding-top:3px;margin-top:2px;">CONCRETE</div></div>`
  ...
  const itemRows = (quotation.items || []).map((item) => `
    <tr>
      ...
      <td style="padding:12px 16px;border-bottom:1px solid #f0f0f0;border-right:1px solid #eee;font-size:13px;text-align:center;">${String(item.qty).padStart(2,'0')}</td>
      <td style="padding:12px 16px;border-bottom:1px solid #f0f0f0;border-right:1px solid #eee;font-size:13px;text-align:center;">${fmt(item.price)}</td>
      <td style="padding:12px 16px;border-bottom:1px solid #f0f0f0;font-size:13px;text-align:center;">${fmt(item.subtotal ?? item.qty * item.price)}</td>
    </tr>`).join('')
  ...
  const validUntil = quotation.validUntil ? String(quotation.validUntil).split('T')[0] : null

  return `<!DOCTYPE html>
<html lang="es"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Cotización ${quotation.quoteNumber}</title></head>
<body style="margin:0;padding:0;background:#f5f5f5;font-family:Arial,Helvetica,sans-serif;">
<div style="max-width:700px;margin:20px auto;background:white;box-shadow:0 2px 12px rgba(0,0,0,.15);">
  <div style="background:${concreteBg};padding:40px 48px 32px;display:flex;justify-content:space-between;align-items:flex-start;">
    <div>
      <div style="font-size:46px;font-weight:900;color:${green};letter-spacing:-1px;line-height:1;">COTIZACIÓN</div>
      <div style="margin-top:12px;background:white;border:1.5px solid #888;padding:5px 20px;display:inline-block;">
        <span style="font-size:15px;font-weight:700;color:#222;">Nº: ${quotation.quoteNumber}</span>
      </div>
    </div>
    <div style="text-align:right;">${logoHtml}</div>
  </div>
  <div style="padding:36px 48px;display:flex;border-bottom:1px solid #e0e0e0;">
    ...
    <div style="flex:1;padding-left:40px;text-align:right;">
      <p style="font-weight:800;font-size:15px;margin:0 0 12px;color:#111;text-transform:uppercase;">${settings.companyName || 'Amazonia Concrete'}</p>
      ${validUntil ? `<p style="font-size:13px;margin:5px 0;color:#555;">Válida hasta: ${validUntil}</p>` : ''}
      ${quotation.deliveryEstimate ? `<p style="font-size:13px;margin:5px 0;color:#555;">Entrega: ${quotation.deliveryEstimate}</p>` : ''}
    </div>
  </div>
  <div style="padding:32px 48px 16px;">
    <table style="width:100%;border-collapse:collapse;border:1.5px solid #333;">
      <thead><tr style="border-bottom:1.5px solid #333;background:#f5f5f5;">
        <th style="text-align:left;padding:12px 18px;font-size:14px;font-weight:700;border-right:1px solid #bbb;">Detalle</th>
        <th style="text-align:center;padding:12px 16px;font-size:14px;font-weight:700;border-right:1px solid #bbb;width:100px;">Cantidad</th>
        <th style="text-align:center;padding:12px 16px;font-size:14px;font-weight:700;border-right:1px solid #bbb;width:140px;">Precio</th>
        <th style="text-align:center;padding:12px 16px;font-size:14px;font-weight:700;width:140px;">Total</th>
  ...
  <div style="padding:16px 48px 8px;text-align:right;font-size:13px;color:#444;">
    <div style="margin:4px 0;">Subtotal: <strong>${fmt(quotation.subtotal ?? 0)}</strong></div>
    <div style="margin:4px 0;">IVA: <strong>${fmt(quotation.tax ?? 0)}</strong></div>
  </div>
  <div style="padding:8px 48px 24px;text-align:right;">
    <table style="border:2px solid #222;min-width:280px;margin-left:auto;">
      <tr><td style="padding:13px 24px;font-weight:800;font-size:15px;letter-spacing:1.5px;">TOTAL</td>
      <td style="padding:13px 24px;text-align:right;font-weight:700;font-size:15px;border-left:1px solid #ccc;">${fmt(quotation.total)}</td></tr>
    </table>
  </div>
  ${quotation.notes ? `<div style="padding:0 48px 20px;font-size:13px;color:#444;"><strong>Notas:</strong> ${quotation.notes}</div>` : ''}
  <div style="text-align:center;padding:6px 48px 20px;font-size:13px;color:${green};font-style:italic;font-weight:600;">
    Gracias por tu interés en ${settings.companyName || 'Amazonia Concrete'} 🌿
  </div>
  <div style="background:${darkBg};padding:24px 48px;display:flex;justify-content:space-between;align-items:center;">
    <div style="color:white;">${socialLinks || '<span style="color:#888;font-size:12px;">Amazonia Concrete</span>'}</div>
    <div style="border-left:3px solid #666;padding-left:18px;color:#ddd;font-size:15px;font-style:italic;font-weight:300;">
      ${settings.slogan || 'Belleza natural en concreto'}
    </div>
  </div>
</div></body></html>`
...
router.post('/quotation', async (req, res) => {
  const { quotation, customer, recipientEmail, pdfBase64 } = req.body
  if (!recipientEmail) return res.status(400).json({ error: 'Se requiere el correo del destinatario' })
  if (!quotation) return res.status(400).json({ error: 'Falta la cotización' })
  ...
  const cust = customer || { name: quotation.customer, email: recipientEmail, phone: '' }
  const html = buildQuotationHtml({ quotation, customer: cust, settings })
  ...
  try {
    ...
    const u = getUser(req)
    await log({
      userName: u.name, userEmail: u.email, action: 'crear', entity: 'Cotización',
    ...
    const raw = e.message || String(e)
    let friendly = raw
    if (/Invalid login|535|EAUTH|Username and Password not accepted/i.test(raw))
      friendly = 'Usuario o contraseña SMTP incorrectos. Si usas Gmail, genera una "Contraseña de aplicación".'
